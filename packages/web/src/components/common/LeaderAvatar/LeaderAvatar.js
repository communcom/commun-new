...
const PATTERN = `<svg width="54" height="54" fill="none" xmlns="http://www.w3.org/2000/svg"           preserveAspectRatio="xMidYMid meet" viewBox="0 0 54 54">
  <mask id="_ma">
...
const LeaderAvatar = forwardRef(({ userId, percent, useLink, avatarSize, ...props }, ref) => {
  let d = '';
  ...
  return (
    <Wrapper as={useLink ? 'a' : undefined} {...props} ref={ref}>
      <BorderWrapper
        className="leaders-avatar__rating"
        dangerouslySetInnerHTML={{
          __html: svg,
        }}
      />
      <AvatarStyled userId={userId} size={avatarSize} />
    </Wrapper>
  );
});
